//! Advanced private network demo: approve one peer, deny another.
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("Bob (approved) public key: {}", hex_encode(&bob_key.public));
    ...
    // Spawn host join handler with conditional approval
    let alice_clone = alice.clone();
    let bob_key_cloned = bob.public_key().clone();
    tokio::spawn(async move {
        loop {
            if let Some((net_id_req, joiner)) = alice_clone.next_host_join_request().await {
                ...
                // Approve Bob, deny Charlie
                let approved = joiner.pubkey == bob_key_cloned;
                if let Err(e) = alice_clone
                    .reply_join_request(net_id_req, joiner.pubkey, approved)
                    .await
                {
                    info!("Alice: failed to reply: {}", e);
                } else {
                    info!("Alice: {}", if approved { "approved" } else { "denied" });
                }
            }
    ...
    // Bob joins (should be approved)
    info!("Bob joining private network...");
    match bob.join_network(net_id).await {
        Ok(()) => info!("✅ Bob joined network (approved)"),
        Err(e) => info!("❌ Bob failed: {}", e),
    }

    // Charlie joins (should be denied)
    info!("Charlie joining private network...");
    match charlie.join_network(net_id).await {
        Ok(()) => info!("❌ Charlie joined network (unexpected)"),
        Err(e) => info!("✅ Charlie refused: {}", e),
    }
    ...
    info!("Private network advanced test completed successfully.");
    Ok(())
}
